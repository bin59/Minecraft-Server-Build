...
Vault 是连接本服各玩法插件与经济、权限核心的 API 桥，本文档记录它与 EssentialsX、LuckPerms 的安装顺序，以及 Residence 买卖、死亡收费、QuickMenu 菜单、SimplePets 宠物等插件如何经 Vault 统一调用同一套货币。面向负责服务器经济与权限对接的管理员。
...
> **本服账本**：`plugins/Vault.jar` 为标准 Vault API 桥（`main: net.milkbowl.vault.Vault`），全服经济账本统一在 EssentialsX——Residence / CustomDeathMessages / SimplePets / DeluxeMenus / PlaceholderAPI 全部走 Vault API 读同一本账，货币为「南瓜币」。
...
CDM 可选接入 Vault，每次展示死亡消息向玩家扣费；指定 LuckPerms 组（如 `admin`）免单。
...
配置：在 Addon 配置里给宠物标价；玩家用 `/pet purchased` 查看已购；免单权限 `Pet.economy.bypass` 给管理组（admin）即可。
