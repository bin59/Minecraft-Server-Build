...
| 插件             | 作者 / 出处                                                    | 定位                      | 一句话画像                                                                                                                                         |
| -------------- | ---------------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **FarPets**    | MavenPL（波兰），SpigotMC #138745                               | 今日刚发布的 "装饰 + 轻度养成" 宠物插件 | **2026-09-19 当天才上架**，v1.0.2，仅 14 次下载、0 评分；50+ 原版宠物、22 种被动加成、等级经验、70+ 粒子，但**无 Vault、无自定义模型、无开源仓库**。                                            |
...
| **1.21.11 官方点名**   | 未点名（"1.21+" 推断可跑，需自测）                 | 未点名（"1.20.4+" 推断可跑，需自测）                                        | **明确在支持列表内**（R5-B311 起）                                                     |
...
## 四、维护状态（截至 2026-09-19）



| 维度      | FarPets                 | RZXPets                            | SimplePets（R5）                                              |
| ------- | ----------------------- | ---------------------------------- | ----------------------------------------------------------- |
| 首发时间    | **2026-09-19（今天）**      | 2026-07-26 / 27                    | 2015 年起（v4 时代），R5 重写版持续在更                                   |
| 最新版本    | 1.0.2（今天下午紧急修复飞行骑乘 bug） | 1.0.1（2026-07-27）                  | **R5-B315（2026-09-06）**                                     |
...
| 社区反馈样本  | 无（今天才发布）                | 无（2 个月、零讨论）                        | 旧 v4 遗留抱怨：跨世界卡住、与 Citizens 冲突、某次更新宠物全丢（均已停更版本）              |
...
* 今天刚上架、**完全免费、无内购**（[SpigotMC #138745](https://www.spigotmc.org/resources/farpets.138745/)）。
...
* **1.0 阶段、当天 14 次下载、0 评分**，作者主语言波兰语，配置键 `ogolne` / `pety-niesmiertelne` 为波兰语；无 Wiki、Documentation 标签死链、PAPI 占位符清单未公开。

* **无 Vault**，与你现有经济体系无法直接打通；无自定义模型 / MythicMobs；Leaf、Geyser、Folia 均未表态。

* 首发当天即热修两个 bug（飞行骑乘卡空、宠物无敌化默认开启），早期版本稳定性未经社区验证。
...
* 发布约 2 个月：下载～24、GitHub 0 star / 0 issue / 0 fork，社区零反馈；License 是 All-Rights-Reserved（禁商用、禁再分发），商用服需自行评估。

* 作者自曝已修早期 bug：Bat 实体 `setAI(false)` 冻结、和平难度刷掉 Blaze 类宠物、区块卸载后宠物不重生、升级 GUI 被主菜单覆盖。

### 5.3 SimplePets（brainsynder / BSDevelopment，当前在你服运行的版本）

**亮点**



* 十年积累，**R5 官方版本表明确含 1.21.11**（最新构建 R5-B315，2026-09-06，支持 1.21.8 \~ 26.2），Java 21，Paper/Purpur 分支直接跑。

* 90 种原版生物，**骑乘 / 飞行 / 顶头 / 多只同放 / Pet Saves 存盘 / 五种改名方式 / 每宠 JSON 微调外观**（幼年、羊毛色、项圈色、史莱姆尺寸、荧光、手持方块等）。

* 权限粒度细到 `pet.type.<mob>.{fly,mount,hat,data.*}`；Vault Addon 免费、支持一次性购买或按次召唤付费，正好对接你已有的 Vault 主货币体系。

* GPL-3.0 开源，Modrinth \~24.9K 下载，旧版 10 万 + 下载、4.1/5 口碑。
...
1. **兼容性这一票只有 SimplePets 稳。** 你的目标版本是 Leaf 1.21.11（Paper 分支），三者中**只有 SimplePets 在官方版本表把 1.21.11 列了出来**；FarPets 只写 "1.21+"，RZXPets 写 "1.20.4+"，二者对 1.21.11 都是推断，需要你自己吃测试成本。

2. **经济已经打通，换了要重做。** 你现网已经跑着 SimplePets + Vault Addon，QuickMenu 里 `/pet gui` 直达也接好了；FarPets **完全没有 Vault 集成**，RZXPets 虽然接 Vault 但强依赖未公开的 RZXCore—— 切换都要重新接经济、重做菜单入口、重新配宠物定价与权限组。

3. **成熟度差距悬殊。** SimplePets 十年历史、Modrinth 24.9K 下载、旧版 217 条 4.1 分评价；FarPets 是**今天才上架的 v1.0.2**（14 次下载、0 评论），RZXPets 发布 2 个月、24 次下载、GitHub 0 star / 0 issue。这两个插件目前没有经过任何第三方生产环境验证，直接上生存服等于当测试员。

4. **功能并不缺你要的。** 你的诉求是 "外观奖励型装饰宠物 + 经济购买"，SimplePets 的骑乘 / 飞行 / 顶头 / 多宠同放 / 每宠定价已覆盖；FarPets 多出来的 "等级 + 22 种被动加成" 和 RZXPets 的 "100 级 buff" 属于 RPG 养成方向，而你的 QuickMenu 定位是主菜单里的休闲直达项，不是 RPG 养成线 —— 为这个未确认的方向换掉一个已经跑稳的插件不划算。

5. **三者共同的天花板，换谁都解决不了。** 它们**都不支持** ModelEngine / ItemsAdder / Oraxen / MythicMobs 自定义模型宠物，**都没有官方 Geyser/Floodgate 适配声明**（基岩端 GUI 交互都要你实测），**都不官方支持 Folia**。也就是说，换 FarPets / RZXPets 不仅没拿到新东西，还丢掉了成熟度和经济链路。
...
* 若未来真的想要 "养成型宠物"（等级 / 技能树 / 自定义模型），本次三选一里没有合适答案，应另开一轮调研 **PetBlocks**（社区公认支持 Geyser/Folia/1.21.x）或 **AdvancedPets / MyPet** 这类成熟 RPG 向宠物插件。

* 基岩端玩家在 `/pet gui` 容器里的点击、改名弹窗（铁砧 / 对话 / 告示牌）在 Floodgate 下的表现，建议在测试服用基岩版账号实测一遍 —— 三家都没官方承诺，这条是你自己要承担的验证项。
...
* 社区口碑参考：Spigot 旧版 v4.4 评论区 [https://www.spigotmc.org/resources/simplepets-outdated-read-description.14124/reviews](https://www.spigotmc.org/resources/simplepets-outdated-read-description.14124/reviews) ；issue 统计 [https://issues.ecosyste.ms/hosts/GitHub/repositories/brainsynder-Dev/SimplePets](https://issues.ecosyste.ms/hosts/GitHub/repositories/brainsynder-Dev/SimplePets)

> 注：Modrinth / Hangar / GitHub 上的下载量、star、版本号为 2026-09-19 当日抓取值，会随时间变化；商业价格与授权以 voxel.shop、SpigotMC、GitHub LICENSE 原文为准。